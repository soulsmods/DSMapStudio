...
void main()
{
	...
	vec4 diffuseColor = texture(sampler2D(globalTextures[nonuniformEXT(int(matdata[fsin_mat].colorTex))], anisoLinearSampler), fsin_texcoord.xy); //vec3(0.5) * vec3(0.5);
	...
	float roughness = 1.0 - (normalColor.z * shininessColor.r);
