...
namespace StudioCore
{
    ...
    {
        private static readonly Dictionary<GameType, FromSoftGame> gameMapping = new Dictionary<GameType, FromSoftGame>
        {
        ...
        public async override Task<ServerInfoResponse> GetServerInfo(ServerCallContext context)
        {
            ServerInfoResponse response = new ServerInfoResponse
            {
                Id = "DSMapStudio",
                Version = version,
                ServerPath = System.Diagnostics.Process.GetCurrentProcess().MainModule?.FileName,
            };

            if (assetLocator.GameModDirectory != null
                && gameMapping.TryGetValue(assetLocator.Type, out FromSoftGame gameType))
            {
                EditorResource projectResource = new EditorResource
                {
                ...
                if (msbEditor.Universe.LoadedObjectContainers.Count > 0)
                {
                    foreach (KeyValuePair<string, ObjectContainer> entry in msbEditor.Universe.LoadedObjectContainers)
                    {
                        if (entry.Value != null)
                        {
                            EditorResource mapResource = new EditorResource
                            {
                ...
                {
                    EditorResource textResource = new EditorResource
                    {
        ...
        private static object AccessMapProperty(MapEntity e, string key)
        {
            ...
            object val = e.GetPropertyValue(key);
            if (val == null)
            {
                return null;
            }
        ...
        private static object AccessFmgProperty(SoulsKey.FmgKey fmgKey, SoulsFmg.FmgKeyInfo keyInfo, FMG.Entry entry, string key)
        {
            switch (key)
            {
                case "ID":
        ...
        public async override Task<IEnumerable<SoulsObject>> GetObjects(
            ServerCallContext context,
            EditorResource resource,
            List<SoulsKey> keys,
            RequestedProperties properties)
        {
            List<SoulsObject> results = new List<SoulsObject>();
                ...
                && ParamBank.PrimaryBank?.Params is IReadOnlyDictionary<string, Param> paramDict)
            {
                foreach (SoulsKey getKey in keys)
                {
                    ...
                    {
                        SoulsObject obj = new SoulsObject(gameParamKey);
                    ...
                    else if (getKey is SoulsKey.GameParamRowKey gameParamRowKey)
                    {
                        ...
                        if (row != null)
                        {
                            SoulsObject obj = new SoulsObject(gameParamRowKey);
                ...
                && MatchesResource(resource, lang.ToString()))
            {
                foreach (SoulsKey getKey in keys)
                {
                    ...
                    FMGBank.FMGInfo info = FMGBank.FmgInfoBank
                        .Find(info => GetFmgKey(game, lang, info, out SoulsKey.FmgKey infoKey) && infoKey.Equals(fileKey));
                    if (info == null)
                    {
                        continue;
                    }
                    ...
                    {
                        SoulsObject obj = new SoulsObject(fmgKey);
                    ...
                    else if (getKey is SoulsKey.FmgEntryKey fmgEntryKey)
                    {
                        ...
                        if (entry != null)
                        {
                            SoulsObject obj = new SoulsObject(fmgEntryKey);
                            obj.AddRequestedProperties(properties, key => AccessFmgProperty(fileKey, keyInfo, entry, key));
                            results.Add(obj);
                        }
                    }
                }
            ...
            if (resource.Type == EditorResourceType.Map)
            {
                foreach (SoulsKey getKey in keys)
                {
                    if (getKey.File is not SoulsKey.MsbKey fileKey
                        || !msbEditor.Universe.LoadedObjectContainers.TryGetValue(fileKey.Map, out ObjectContainer container)
                    ...
                    {
                        SoulsObject obj = new SoulsObject(msbKey);
                    ...
                    else if (getKey is SoulsKey.MsbEntryKey msbEntryKey && container is Map m)
                    {
                        ...
                        {
                            if (ob is not MapEntity e || !mapNamespaces.TryGetValue(e.Type, out KeyNamespace ns) || ns != msbEntryKey.Namespace)
                            {
                                continue;
                            }
                            SoulsObject obj = new SoulsObject(msbEntryKey);
        ...
        public async override Task<IEnumerable<SoulsObject>> SearchObjects(
            ServerCallContext context,
            EditorResource resource,
            ...
            if (resource.Type == EditorResourceType.Param)
            {
                ...
                foreach (KeyValuePair<string, Param> entry in ParamBank.PrimaryBank?.Params ?? new Dictionary<string, Param>())
                {
                    ...
                    if (resultType.Matches(typeof(SoulsKey.GameParamKey)))
                    {
                        ...
                        SoulsObject obj = new SoulsObject(fileKey);
                    ...
                    if (resultType.Matches(typeof(SoulsKey.GameParamRowKey)))
                    {
                        foreach (Param.Row row in entry.Value.Rows)
                        {
                            if (search.IsMatch(key => AccessParamProperty(entry.Key, row, key)))
                            {
                                ...
                                SoulsObject obj = new SoulsObject(new SoulsKey.GameParamRowKey(fileKey, row.ID));
                ...
                foreach (FMGBank.FMGInfo info in FMGBank.FmgInfoBank)
                {
                    ...
                    if (resultType.Matches(typeof(SoulsKey.FmgEntryKey)))
                    {
                        foreach (FMG.Entry entry in info.Fmg.Entries)
                        {
                            ...
                            {
                                SoulsObject obj = new SoulsObject(new SoulsKey.FmgEntryKey(fileKey, entry.ID));
                                obj.AddRequestedProperties(properties, key => AccessFmgProperty(fileKey, keyInfo, entry, key));
                                if (addResult(obj))
                                {
                                    goto limitReached;
                ...
                foreach (KeyValuePair<string, ObjectContainer> entry in msbEditor.Universe.LoadedObjectContainers)
                {
                    ...
                    if (resultType.Matches(typeof(SoulsKey.MsbEntryKey)) && entry.Value is Map m)
                    {
                        ...
                        foreach (Entity ob in m.Objects)
                        {
                            ...
                            {
                                SoulsObject obj = new SoulsObject(new SoulsKey.MsbEntryKey(fileKey, ns, e.Name));
                                obj.AddRequestedProperties(properties, key => AccessMapProperty(e, key));
                                if (addResult(obj))
                                {
                                    goto limitReached;
        ...
        public async override Task OpenResource(ServerCallContext context, EditorResource resource)
        {
        ...
        public async override Task OpenObject(
            ServerCallContext context,
            EditorResource resource,
            SoulsKey key)
        {
            ...
            if (resource.Type == EditorResourceType.Map)
            {
                if (key is SoulsKey.MsbKey msbKey && MatchesResource(resource, msbKey.Map))
                {
                    ...
                    && revMapNamespaces.TryGetValue(msbEntryKey.Namespace, out MapEntity.MapEntityType entityType))
                {
                    EditorCommandQueue.AddCommand(new[] { "map", "select", msbEntryKey.File.Map, msbEntryKey.Name, entityType.ToString() });
                    EditorCommandQueue.AddCommand("windowFocus");
                }
            }
            else if (resource.Type == EditorResourceType.Param)
            {
                ...
                {
                    EditorCommandQueue.AddCommand(new[] { "param", "select", "-1", paramRowKey.File.Param, paramRowKey.ID.ToString() });
                    EditorCommandQueue.AddCommand("windowFocus");
                }
            }
            ...
            {
                string commandKey = fmgKeyInfo.BinderID >= 0 ? fmgKeyInfo.BinderID.ToString() : fmgKeyInfo.FmgName;
        ...
        public async override Task OpenSearch(
            ServerCallContext context,
            EditorResource resource,
            SoulsKeyType resultType,
            PropertySearch search,
            bool openFirstResult)
        {
            ...
            if (resource.Type == EditorResourceType.Map)
            {
                if (resultType.Matches(typeof(SoulsKey.MsbEntryKey)))
                {
                    ...
                    {
                        List<string> cmd = new List<string> { "map", "propsearch", cond.Key, cond.Value.ToString() };
                        if (openFirstResult)
                        {
