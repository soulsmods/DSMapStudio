    ...
    public class DbgPrimWireSpotLight : DbgPrimWire
    {
        ...
        public DbgPrimWireSpotLight(Transform location, float radius, float angle, Color color)
        {
            ...
            else
            {
                ...
                var topPoint = Vector3.UnitY * sphereRadius;
                var bottomPoint = -Vector3.UnitY * sphereRadius;
                ...
                for (int i = 0; i < numVerticalSegments; i++)
                {
                    for (int j = 0; j < numSidesPerSegment; j++)
                    {
                        ...
                        points[i, j] = new Vector3((float)Math.Cos(horizontalAngle) * horizontalDist, altitude, (float)Math.Sin(horizontalAngle) * horizontalDist) * sphereRadius;
                    }
                }
